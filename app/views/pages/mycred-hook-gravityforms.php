	...
	if ( ! class_exists( 'myCRED_Gravity_Forms' ) && class_exists( 'myCRED_Hook' ) ) {
		class myCRED_Gravity_Forms extends myCRED_Hook {
			...
			public function run() {
				...
				add_action( 'gform_after_update_entry', array( $this, 'form_edit' ), 10, 2 );
			}
			...
			public function form_submission( $lead, $form ) {
                ...
                if($form['title']=="Reservation" || $form['title']=="Member get Member"){return;}
				// Prep
			 ...
			 * @version 1.1
			 */
			public function form_edit( $lead, $form ) {

				// Prep
				$lead_id = absint( $form );
				$leadModel = RGFormsModel::get_lead( $lead_id);
				// echo json_encode($leadModel);
				// echo "<br/>";
				// echo var_dump($lead['created_by']);
				$user_id = absint( $leadModel['created_by'] );
				$form_id = absint( $leadModel['form_id'] );
				//echo $user_id."----".$form_id;
				//echo "<br/>";
				//echo "<br/>";
				//echo $lead['title']."----".$leadModel['3'];
				//echo "<br/>";
				//echo $lead['title']."----".$leadModel['5'];
				// echo "<br/>";
				// echo "<br/>";
				// echo json_encode($lead);

				if ( ! isset( $lead['title'] ) ) {return;}
				// Default values
				$amount = $this->prefs[ $form_id ]['creds'];
				$entry = $this->prefs[ $form_id ]['log'];
				...
				$status = "";
				if($lead['title']=="Reservation" || $lead['title']=="Member get Member"){ 
				}else{
					return;
				}
				if($leadModel['5']=="Approve" || $leadModel['3']=="Approve"){
